...
def plot_hiv_burden_by_sex(hiv, hiv_proj_sex, ax, fig):
    ...
    proj = (
        hiv_proj_sex.filter(pl.col("year") >= 2025)
        .group_by(["year", "sex"])
        .agg(pl.sum("value").alias("incidence"))
        .sort("year")
    )
    ax.plot(
        proj.filter(pl.col("sex") == "Male")["year"],
    ...
    ax.plot(
        proj.filter(pl.col("sex") == "Female")["year"],
    ...
    ax.yaxis.set_minor_locator(
        __import__("matplotlib").ticker.MultipleLocator(100000)
    )
    ax.xaxis.set_minor_locator(
        __import__("matplotlib").ticker.MultipleLocator(2)
    )
...
def plot_hiv_burden_by_region(hiv, hiv_proj, ax, fig):
    ...
    proj = (
        hiv_proj.filter(pl.col("year") >= 2025)
    ...
    ax.set_ylim(0)
    ax.yaxis.set_minor_locator(
        __import__("matplotlib").ticker.MultipleLocator(50000)
    )
    ax.xaxis.set_minor_locator(
        __import__("matplotlib").ticker.MultipleLocator(2)
    )
