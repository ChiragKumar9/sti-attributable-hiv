...
def calculate_adult_by_subtraction(data, measure_name):
    ...
    adult = (
        all_ages.join(child, on=["country", "year", "sex"], how="inner")
        .with_columns(
            age=pl.lit("adult"),
...
population_adults = (
    ...
    .group_by(["measure", "location", "sex", "year"])
    .agg(pl.sum("val"), pl.sum("upper"), pl.sum("lower"))
    .with_columns(age=pl.lit("adult"))
)

# Keep child (0-14) and all ages as-is, rename
population_other = (
    population.filter(pl.col("age").is_in(["0-14 years", "All ages"]))
    .with_columns(
    ...
    .select(
        ["measure", "location", "sex", "year", "val", "upper", "lower", "age"]
    )
)

# Combine adult, child, all ages
population = pl.concat([population_adults, population_other])
population = population.rename({"location": "country", "val": "value"}).select(
...
incidence_hiv = pl.read_csv(
    os.path.join(data_dir, "UNAIDS_HIV_incidence_rate.csv")
)
...
incidence_hiv = incidence_hiv.filter(pl.col("sex_count") == 3).drop(
    "sex_count"
)
...
incidence_hiv = incidence_hiv.join(
    country_codes, left_on="location", right_on="alpha-3", how="left"
)
...
incidence_hiv_by_age = pl.read_csv(
    os.path.join(data_dir, "UNAIDS_HIV_incidence_num.csv")
)
...
incidence_adult_calc = calculate_adult_by_subtraction(
    incidence_both_only, "incidence_number"
)
...
incidence_hiv_by_age = pl.concat(
    [incidence_hiv_by_age, incidence_child_male, incidence_child_female]
)

# DATASET 4: HIV PREVALENCE (by age/sex)
prevalence_hiv = pl.read_csv(
    os.path.join(data_dir, "UNAIDS_HIV_prevalence.csv")
)
...
prevalence_hiv = prevalence_hiv.with_columns(
    ...
    all_ages_Male_lower=pl.col("child_Male_lower")
    + pl.col("adult_Male_lower"),
    all_ages_Male_upper=pl.col("child_Male_upper")
    + pl.col("adult_Male_upper"),
    all_ages_Female_val=pl.col("child_Female_val")
    + pl.col("adult_Female_val"),
    all_ages_Female_lower=pl.col("child_Female_lower")
    + pl.col("adult_Female_lower"),
    all_ages_Female_upper=pl.col("child_Female_upper")
    + pl.col("adult_Female_upper"),
    all_ages_Both_val=pl.col("child_Both_val") + pl.col("adult_Both_val"),
    all_ages_Both_lower=pl.col("child_Both_lower")
    + pl.col("adult_Both_lower"),
    all_ages_Both_upper=pl.col("child_Both_upper")
    + pl.col("adult_Both_upper"),
)
...
combined_data = pl.concat(
    [population, incidence_hiv, incidence_hiv_by_age, prevalence_hiv_long]
)
...
combined_data = combined_data.filter(
    pl.col("country").is_in(countries_to_keep)
)
...
inc_num_vals = filter_measure_and_alias(
    combined_data, "incidence_number", "inc"
)

uninfected_calc_1 = pop_vals.join(
    prev_vals, on=["country", "year", "sex", "age"], how="inner"
)
uninfected_calc_1 = uninfected_calc_1.join(
    inc_num_vals, on=["country", "year", "sex", "age"], how="inner"
)

uninfected_pop_1 = uninfected_calc_1.with_columns(
    measure=pl.lit("uninfected_pop"),
    value=pl.col("pop_value")
    - pl.col("prev_value")
    - 0.5 * pl.col("inc_value"),
    upper=pl.col("pop_upper")
    - pl.col("prev_lower")
    - 0.5 * pl.col("inc_lower"),
    lower=pl.col("pop_lower")
    - pl.col("prev_upper")
    - 0.5 * pl.col("inc_upper"),
).select(
    ["country", "year", "sex", "age", "measure", "value", "upper", "lower"]
)

# Formula 2: Using incidence_rate (all ages Male/Female only)
inc_rate_vals = filter_measure_and_alias(
    combined_data, "incidence_rate", "inc_rate"
)

uninfected_calc_2 = pop_vals.join(
    prev_vals, on=["country", "year", "sex", "age"], how="inner"
)
uninfected_calc_2 = uninfected_calc_2.join(
    inc_rate_vals, on=["country", "year", "sex", "age"], how="inner"
)

# Only keep all ages Male/Female
uninfected_calc_2 = uninfected_calc_2.filter(
    (pl.col("age") == "all ages") & (pl.col("sex").is_in(["Male", "Female"]))
)

uninfected_pop_2 = uninfected_calc_2.with_columns(
    measure=pl.lit("uninfected_pop"),
    value=(pl.col("pop_value") - pl.col("prev_value"))
    / (1 + 0.5 * pl.col("inc_rate_value")),
    upper=(pl.col("pop_upper") - pl.col("prev_lower"))
    / (1 + 0.5 * pl.col("inc_rate_lower")),
    lower=(pl.col("pop_lower") - pl.col("prev_upper"))
    / (1 + 0.5 * pl.col("inc_rate_upper")),
).select(
    ["country", "year", "sex", "age", "measure", "value", "upper", "lower"]
)
...
uninf_male_female = uninfected_pop.filter(
    pl.col("sex").is_in(["Male", "Female"])
)
uninf_adult_mf = calculate_adult_by_subtraction(
    uninf_male_female, "uninfected_pop"
)
...
uninfected_pop_vals = filter_measure_and_alias(
    combined_data, "uninfected_pop", "unin_pop"
)
inc_vals = filter_measure_and_alias(combined_data, "incidence_number", "inc")

incidence_rate_calc = uninfected_pop_vals.join(
    inc_vals, on=["country", "year", "sex", "age"], how="inner"
)
...
incidence_num_calc = uninfected_pop_vals.join(
    inc_rate_vals, on=["country", "year", "sex", "age"], how="inner"
)
...
incidence_numbers = incidence_numbers.filter(
    pl.col("sex").is_in(["Male", "Female"])
)
...
inc_num_adult_mf = calculate_adult_by_subtraction(
    inc_num_male_female, "incidence_number"
)
...
incidence_rate_final_calc = uninfected_pop_vals.join(
    inc_num_vals, on=["country", "year", "sex", "age"], how="inner"
)

# Only keep adult Male/Female
incidence_rate_final_calc = incidence_rate_final_calc.filter(
    (pl.col("age") == "adult") & (pl.col("sex").is_in(["Male", "Female"]))
)
...
prevalence_gc = prevalence_gc.group_by(["location", "sex", "year"]).agg(
    pl.sum("val"), pl.sum("lower"), pl.sum("upper")
)
